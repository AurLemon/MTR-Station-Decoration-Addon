...
SERVER_DIR="${SERVER_DIR:-$ROOT_DIR/server-smoke-block-entity}"
...
SERVER_PORT="${SERVER_PORT:-$((30000 + (BASHPID % 2000)))}"
...
send_command 'msd_probe_block_entity yamanote 58 64 0 "11,22" "platform,station"'
wait_for_log 'MSD yamanote sign updated'
...
send_command 'say probe-yamanote-config-ok'
wait_for_log 'probe-yamanote-config-ok'
...
wait_for_log '45.0'
send_command 'say probe-model-config-ok'
wait_for_log 'probe-model-config-ok'

send_command 'stop'
wait "$SERVER_PID"
SERVER_PID=""

echo "Block entity config probe passed. Log: $LOG_FILE"
